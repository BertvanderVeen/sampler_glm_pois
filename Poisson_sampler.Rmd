...
```{r my_post_calc}
calc_post_mom <- function(X = double(2), n.response.nodes = integer(), n.zero.response.nodes = integer(), response.nodes = integer(1), R1 = integer(1), R2 = integer(1), tau = double(2), mcompv = double(2), mcompm = double(2), my = double(2), vy = double(2)){
    X <- as.matrix(X)
...
test it:
```{r test_post_calc}
set.seed(1);my_post <- calc_post_mom(cbind(1,x), N, sum(y==0), y, pogit_R[1:N], pogit_R[-c(1:N)], my_tau, mcomp$v, mcomp$m, compmix.pois$my, compmix.pois$vy)
...
tau <- update_tau(response.nodes, N, n.zero.response.nodes, lambda) # intitialize tau
R1 <- update_R1(N, lp, tau[,1], mcomp$m, mcompv = mcomp$v, c1)# initialize R1
R2 <- update_R2(response.nodes, N, n.zero.response.nodes, lp, tau[y>0,2], vy, my, wy)  # initialize R2
pb = txtProgressBar(min = 0, max = n.iter, initial = 0) # progress bar. because, why not!

for(i in 1:n.iter){
# calculate posterior moments
  calc_post_mom(X, N, n.zero.response.nodes, y, R1, R2, tau, mcomp$v, mcomp$m, my, vy)
  ...
  lambda = exp(lp) # distributional parameter
  tau <- update_tau(response.nodes, N, n.zero.response.nodes, lambda) # update tau
  R1 <- update_R1(N, lp, tau[,1], mcomp$m, mcompv = mcomp$v, c1)# update R1
  R2 <- update_R2(response.nodes, N, n.zero.response.nodes, lp, tau[y>0,2], vy, my, wy)  # update R2
