...
```{r my_post_calc}
calc_post_mom <- function(X = double(2), n.response.nodes = integer(), n.zero.response.nodes = integer(), n.param.nodes = integer(), response.nodes = integer(1), R1 = double(1), R2 = double(1), tau = double(2), mcompv = double(2), mcompm = double(2), my = double(2), vy = double(2), priorMean = double(1), priorCov = double(2)){
    m1 <- mcompm[1,R1]
...
sampler_glm_pois <- nimbleFunction(
  ...
  run = function() {
    ...
    # calculate posterior moments
    calc_post_mom(X, n.response.nodes, n.zero.response.nodes, n.param.nodes, response.nodes, R1, R2, tau, mcompv, mcompm, my, vy, priorMean, priorCov)
  ...
  methods = list(calc_post_mom = function(X = double(2), n.response.nodes = integer(), n.zero.response.nodes = integer(), n.param.nodes = integer(), response.nodes = integer(1), R1 = double(1), R2 = double(1), tau = double(2), mcompv = double(2), mcompm = double(2), my = double(2), vy = double(2), priorMean = double(1), priorCov = double(2)){
    m1 <- mcompm[1,R1]
    m2 <- numeric(n.response.nodes-n.zero.response.nodes)
    for(i in 1:(n.response.nodes-n.zero.response.nodes)){
      m2[i] <- my[i,R2[i]]
